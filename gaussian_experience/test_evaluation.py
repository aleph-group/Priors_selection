...
from experiments_utils import (
    generate_measurements_gaussian_diag,
    compute_test_gaussian_diag,
)
import torch
import numpy as np
...
if test_case == -1:  # test that skrock outputs the correct posterior distribution
    ...
    g = DiagonalWeightedTikhonovPrior(
        torch.tensor(1.0, device=device), torch.ones(d, device=device) / sigmax**2
    )
    dl = DegradedLikelihood(
        ...
        sigma,
        gamma,
        X_init=p.A_A_adjoint(y).to(device).clone(),
        noise=noise,
        sampler=SKROCK,
        sampler_kwargs={"s": 15},
        lam_reg=None,
    ...
    dl_analytical = DegradedLikelihood(
        y,
        g,
        p,
        sigma,
        ...
        sampler_kwargs={
            ...
            "sigma": torch.tensor(
                sigma * sigmax / np.sqrt(sigma**2 + alpha * sigmax**2)
            ).to(device),
        },
        lam_reg=None,
        project=None,
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(nb_samples)):
            trace_analytical.append(
                dl_analytical.sampler(dl_analytical.factor(dl_analytical.y_sub))
            )
        ...
        np.savez(
            os.path.join(fig_folder, "skrock_test.npz"),
...
if test_case == 0 or test_case == 1:
    ...
    file_name = (
        "convergence_skrock.npz" if test_case == 0 else "convergence_analytical.npz"
    )
    torch.manual_seed(0)
    np.random.seed(0)
    sigmax, sigma = 1.0, 0.05
            ...
            for t in range(ntry):
                ...
                if test_case == 0:
                    dl = DegradedLikelihood(
                        ...
                        sigma,
                        gamma,
                        X_init=p.A_adjoint(y).to(device).clone(),
                        noise=noise,
                    ...
                    dl = DegradedLikelihood(
                        ...
                        sampler_kwargs={
                            ...
                            "sigma": torch.tensor(
                                sigma
                                * sigmax
                                / np.sqrt(sigma**2 + alpha.cpu() * sigmax**2)
                            ).to(device),
                        },
                        lam_reg=None,
                        project=None,
                ...
                lik_trace, lik_mean = dl.compute_test(
                    nb_steps,
                ...
                lik_traces[i, l, t] = -(
                    torch.logcumsumexp(lik_trace, 0).cpu().numpy()
                    - torch.log(torch.arange(1, nb_steps + 1, device=device))
                    .cpu()
                    .numpy()
                )
                ...
                evidences2[i, l, t] = (
                    compute_test_gaussian_diag(
                        ...
                        alpha.item(),
                        y.cpu().numpy(),
                        yp=dl.y_add.cpu().numpy(),
                        ym=dl.y_sub.cpu().numpy(),
                        mlog=True,
                    )
                    .cpu()
                    .numpy()
                ...
                print(
                    "p(y+/y-) sampler: {}\n exact: {}\n lik_mean: {}".format(
                        lik_traces[i, l, t][-1], evidences2[i, l, t], lik_mean
                    )
                )
        np.savez(
            os.path.join(save_folder, file_name),
            lik_traces=lik_traces,
            test_exact=evidences2,
...
if test_case == 2 or test_case == 3:
    ...
    save_folder = os.path.join(fig_folder, "convergence_speed")
    file_name = (
        "convergence_dim_skrock.npz"
        if test_case == 2
        else "convergence_dim_analytical.npz"
    )
    torch.manual_seed(0)
    np.random.seed(0)
    sigmax, sigma = 1.0, 0.05
    ...
    for l in range(ndim):
        ...
        for t in range(ntry):
            ...
            evidences2[l, t] = compute_test_gaussian_diag(
                dims[l],
                sigmax,
                sigma,
                alpha.item(),
...
if test_case == 4:
    ...
    with torch.no_grad():
        for i in range(nval):
            ...
            for k in range(ntry):
                ...
                L_g = 1 / sigmax_ex**2
                gamma = torch.tensor(
                    0.98 * 1 / (L_f + L_g), device=device, dtype=torch.float32
                )
                g = DiagonalWeightedTikhonovPrior(
                ...
                exact_vals[i, k] = compute_test_gaussian_diag(
                    d,
                    sigmax_ex,
                    sigma,
                    alpha.cpu().numpy(),
                ...
                for l in range(nsigma):
                    ...
                    # compute exact p(y+/y-)
                    evidences2[i, l, k] = compute_test_gaussian_diag(
                        d,
                        new_sigmax.cpu().numpy(),
                        sigma,
                        alpha.cpu().numpy(),
                        y.cpu().numpy(),
                        yp=dl.y_add.cpu().numpy(),
    ...
    np.savez(
        os.path.join(save_folder, "res_exact.npz"),
        exact_ratio_trace=evidences2,
        exact_ratio_gaussian=evidencesmean,
        sigmas=sigmaxs.cpu().numpy(),
    )
